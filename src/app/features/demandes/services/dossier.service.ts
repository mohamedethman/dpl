...
import {
  ...
  ApiResponse,
  Dossier,
} from "../../dossier/models/dossier-details";
...
export class DossierService {
  ...
  getDossierById(id: number): Observable<Dossier> {
    ...
    return this.http.get<Dossier>(url, { headers }).pipe(
      catchError((error) => {
        console.error(
          `Erreur lors de la récupération du dossier avec l'ID ${id}:`,
          error
        );
        return throwError(
          () =>
            new Error(
              "Une erreur est survenue lors de la récupération des détails du dossier."
            )
        );
      })
    );
