...
@Component({
  selector: "app-dmm-create",
  templateUrl: "./dmm-create.component.html",
  styleUrls: ["./dmm-create.component.scss"],
})
export class DmmCreateComponent implements OnInit {
  title = "amm-frontend";
    ...
    this.dmmService.getSteps().subscribe((response: ApiResponse) => {
      if (response && Array.isArray(response.data)) {
        this.steps = response.data.map((step) => ({
          ...step,
          // Filter out "Contrat" if hideContrat is true
          elements: (step.moduleElements || []).filter(
            (element) =>
              !hideContrat || element.nomElement !== "Contrat de sous traitance"
          ),
        }));
      }
  ...
  submitMedicament() {
    ...
    this.dmmService.submitMedicament(payload).subscribe({
      next: (response) => {
